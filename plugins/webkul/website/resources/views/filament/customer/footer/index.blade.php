<footer class="px-6 py-12 bg-gradient-to-r from-blue-50 via-purple-50 to-blue-100"
	...
	<div class="container max-w-6xl mx-auto">
		<div class="grid grid-cols-1 gap-8 md:grid-cols-3">
			...
			<!-- Useful Links Column -->
			<div class="md:col-span-1">
				<h3 class="mb-4 text-lg font-medium {{ app()->getLocale() === 'ar' ? 'text-right' : 'text-left' }}">
					{{ __('website::filament/app.footer.useful_links') }}</h3>
			...
			<div class="md:col-span-1">
				@if (isset($contacts['email']) || isset($contacts['phone']))
					<h3 class="mb-4 text-lg font-medium {{ app()->getLocale() === 'ar' ? 'text-right' : 'text-left' }}">
						{{ __('website::filament/app.footer.contact_us') }}</h3>

					@if (isset($contacts['email']))
						<div class="mb-2">
							<a href="mailto:{{ $contacts['email'] }}"
								class="flex items-center text-gray-700 hover:text-primary-600 {{ app()->getLocale() === 'ar' ? 'flex-row-reverse justify-end' : '' }}">
				...
				@if (!$socialLinks->isEmpty())
					<h3 class="mb-4 text-lg font-medium {{ app()->getLocale() === 'ar' ? 'text-right' : 'text-left' }}">
						{{ __('website::filament/app.footer.follow_us') }}</h3>

					<div class="flex flex-wrap gap-2 {{ app()->getLocale() === 'ar' ? 'justify-end' : 'justify-start' }}">
						@foreach ($socialLinks as $item)
							<a href="{{ $item->getUrl() }}" class="p-2 text-white bg-gray-800 rounded-full hover:bg-primary-600"
								target="_blank">
								{!! $item->getIcon() !!}
							</a>
						@endforeach
		...
		<!-- Copyright Section -->
		<div
			class="flex flex-col justify-between pt-8 mt-8 border-t border-gray-200 md:flex-row {{ app()->getLocale() === 'ar' ? 'md:flex-row-reverse' : '' }}">
			<div class="text-sm text-gray-600">
				{{ __('website::filament/app.footer.copyright') }} © <a href="https://aliansikode.com/"
					class="text-primary-500" target="_blank">AliansiKode</a>
			</div>

			<div class="mt-2 text-sm text-gray-600 md:mt-0">
				{{ __('website::filament/app.footer.powered_by') }} : <a href="https://aliansikode.com/"
					class="text-primary-500" target="_blank">AliansiKode</a>
			</div>
		</div>
	</div>
